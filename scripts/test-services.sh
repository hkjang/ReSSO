...
start_postgres() {
  docker inspect "$postgres_container" >/dev/null 2>&1 && return
  ...
  for _ in $(seq 1 60); do
    if docker exec "$postgres_container" pg_isready -U resso >/dev/null 2>&1; then
      # The trigram indexes are only created when pg_trgm is present, so the
      # tests that cover that path need it here.
      docker exec "$postgres_container" psql -U resso -d resso \
        -c 'CREATE EXTENSION IF NOT EXISTS pg_trgm' >/dev/null 2>&1 || true
      return
    fi
    sleep 1
  done
  log "PostgreSQL did not become ready"; exit 1
}

seed_directory() {
  local container="$1"
  docker exec -i "$container" ldapadd -x -H ldap://localhost \
    -D "cn=admin,dc=example,dc=test" -w adminpassword >/dev/null <<'LDIF'
...
LDIF
  ...
  seeded="$(docker exec "$container" ldapsearch -x -H ldap://localhost \
    ...
    "(objectClass=inetOrgPerson)" uid | grep -c '^uid:')"
  test "$seeded" -eq 2 || { log "seeded $seeded accounts, expected 2"; exit 1; }
}
...
make_certificates() {
  ...
  openssl req -newkey rsa:2048 -nodes -keyout "$certs/ldap.key" \
    -out "$certs/ldap.csr" -subj "/CN=localhost" 2>/dev/null
  printf 'subjectAltName=DNS:localhost,IP:127.0.0.1\nextendedKeyUsage=serverAuth\n' > "$certs/ext.cnf"
  openssl x509 -req -in "$certs/ldap.csr" -CA "$certs/ca.crt" -CAkey "$certs/ca.key" \
    -CAcreateserial -out "$certs/ldap.crt" -days 30 -sha256 -extfile "$certs/ext.cnf" 2>/dev/null
