...
- 관리자 → API 키: Realm 구성원이 발급한 개인 API 키의 소유자·범위·만료. 대시보드가 "7일 내 만료"로 세는 키를 여기서 그대로 볼 수 있습니다. Secret은 저장되지 않으므로 표시되지 않습니다.

보존 기간이 더 길어야 한다면 표준 출력을 조직의 수집 Agent가 SIEM으로 전달하도록 구성하세요. 관리자 UI의 내장 로그 저장은 장애 분석 편의를 위한 보조 계층입니다.

`clients` 조회가 멈추면 다음 세 줄을 찾으세요. 셋 다 감사 이벤트가 아니라 서버 로그에만 남습니다.
...
| `a CORS origin could not be checked against the Realm's registered ones` | 프로토콜 Endpoint 앞단의 CORS 검사(Token·UserInfo·JWKS 등 `/realms/...` 전체) | 상태 코드와 본문은 바뀌지 않습니다 — 조회가 실패하면 허용으로 기울지 않고 CORS Header(`Access-Control-Allow-Origin` 등)만 빠집니다. 그래서 브라우저 SPA에는 원인이 드러나지 않는 CORS 실패로만 보이고 이쪽 접근 로그에는 200으로 남습니다. 증상이 «등록되지 않은 Origin»과 똑같으므로 RP의 Origin 설정을 의심하기 전에 이 줄이 있는지 먼저 확인하세요. Origin 원문은 남기지 않고 `realm`만 남으므로 어느 Realm의 조회가 멈췄는지는 그 값으로 봅니다 |
...
관리 화면의 사용자 검색과 감사 이벤트의 행위자 검색은 선행 Wildcard를 사용하므로 B-tree 색인을 사용할 수 없습니다. `pg_trgm` 확장을 설치하면 전체 스캔 대신 색인 스캔을 사용합니다.

감사 쪽이 특히 큽니다. 감사 기록은 365일치를 보관하므로 사용률이 높은 설치에서는 사용자 테이블보다 훨씬 커지고, **사고 조사에서 가장 먼저 하는 일이 특정 계정으로 좁히는 것**입니다. 40만 행 기준으로 항목이 다섯 건뿐인 행위자를 찾는 데 색인 없이는 40만 행을 모두 훑어 34ms가 걸렸고, 색인이 있으면 0.1ms입니다. 이 차이는 행 수에 비례해 벌어집니다.
...
- `resso_authorization_errors_total` 발생 — 인가 요청을 처리하지 못하는 상태. **여섯 중 넷은 RP의 `redirect_uri`로 302 `error=server_error`를 실어 보내므로 성공한 인가와 HTTP 상태가 같습니다** — 요청 카운터에도 정상 Redirect로 잡히고 접근 로그도 `status=302`입니다. 이 계열만이 둘을 구분하며, 로그인 화면에 사람이 도달하지 못하는 장애가 여기서만 드러납니다. `stage` 라벨이 어느 단계가 멈췄는지 알려줍니다 — `realm`(경로의 Realm 조회), `client`(`client_id` 조회), `sso_session`(브라우저 세션 조회), `auth_time`(`max_age` 판정을 위한 최근 인증 시각), `authorization_code`(코드 생성), `authorization_request`(로그인 화면으로 넘길 요청 저장). **여섯 값은 모두 이쪽 장애만 셉니다** — 한때 `auth_time`은 예외여서, 데이터베이스가 interval로 만들 수 없을 만큼 큰 `max_age`를 보내면 그 쿼리가 실패해 이 계열이 올라갔습니다. 인증 없는 호출자가 쿼리 파라미터 하나로 정상인 저장소에 대한 경보를 울릴 수 있었다는 뜻입니다. 이제 그런 값은 clamp되므로(`docs/compatibility.md`의 `max_age` 행) 이 계열이 오르면 언제나 이쪽 문제입니다. 앞의 둘은 500으로 나가므로 요청 카운터에도 보이지만, 한 계열로 세어 "처리하지 못한 인가"를 한 번에 볼 수 있게 했습니다. 서버 로그의 `an authorization request could not be served`가 같은 `stage`와 사유를 담습니다(`client`·`realm`은 각자의 기존 로그 문구를 유지합니다).
- `resso_silent_authentications_total{result="login_required"}` 급증 — RP가 `prompt=none` 거절에 다시 `prompt=none`을 보내는 상태(silent SSO 무한 루프). `login_required`는 세션이 없는 브라우저에 대한 **평범한 답**이라 RP는 자기 로그인 화면을 보여 주고 멈춰야 하는데, 그것을 실패로 읽고 다시 시도하는 RP는 브라우저를 이 서비스와 RP 사이에서 Redirect 속도로 오가게 하며 사용자는 화면이 깜빡이는 것만 봅니다. 그 왕복은 두 답 모두 302라 요청 카운터에서 성공한 인가와 구별되지 않습니다 — 이 계열만이 둘을 구분합니다. `result`는 `code`(세션이 있어 코드 발급)와 `login_required` 둘뿐이고 RP별 라벨은 없으므로, 어느 RP인지는 서버 로그의 `a silent authentication found no session to reuse`가 담은 `client_id`와 `remote_ip`로 찾습니다 — 루프는 **한 `remote_ip`에서 같은 `client_id`가 초당 여러 번** 나타나는 모양입니다. 이쪽 장애로 세션을 읽지 못한 요청은 `login_required`가 아니라 `server_error`로 답하며 이 계열이 아니라 `resso_authorization_errors_total{stage="sso_session"}`에 셉니다. 루프 방지 장치(한 탭 세션에 한 번, 로그아웃 뒤 억제, 거절을 주소에 남기기)는 RP 쪽에 있습니다 — `docs/compatibility.md`의 «RP가 silent SSO를 구현할 때».
...
| `REFRESH_TOKEN_REUSE` | Refresh Token이 회전 이후 다시 제시되어 계열이 폐기되었습니다. **같은 Client에 대해 직전에 `TOKEN_REFRESH` `result=PARTIAL`이 있는지 먼저 확인하세요.** 그 기록의 `rotation_rolled_back=false`는 회전을 되돌리지 못해 RP가 이미 회전된 Token을 그대로 들고 있었다는 뜻이며, 그 경우 이 재사용은 유출이 아니라 이 서비스가 만든 것입니다. |
...
| `result=PARTIAL` | 요청은 요구받은 일을 했고 그에 딸린 동작이 실패한 경우입니다. 비밀번호 변경·재설정에서 다른 세션 종료가 실패했거나, 로그아웃에서 세션 폐기가 실패했거나, 로그인은 성공했지만 인가 코드를 발급하지 못했거나, Token 갱신이 실패한 뒤 회전을 되돌리지 못한 경우가 여기에 해당합니다. 상세에 실패 사유가 들어 있습니다. **예외가 하나 있습니다**: `LOGIN_SUCCESS` `result=PARTIAL`의 상세가 `reason=id_token_hint_mismatch`이면 장애가 아니라 정책입니다 — RP가 `id_token_hint`로 지정한 계정과 다른 계정이 로그인해서 인가 코드를 일부러 발급하지 않은 것이고, 사용자에게는 403 `account_mismatch`가 나갑니다. 고칠 것은 없으며, 같은 사람이 반복해서 나타난다면 RP가 오래된 hint를 계속 보내는지 확인하세요. **`result=FAILURE`만 걸어둔 알림은 이 항목을 놓칩니다.** 해당 세션은 살아 있으므로 관리 → 세션에서 직접 종료하세요. 로그아웃 기록의 대상이 비어 있고 상세의 `error`가 `look the session up:`으로 시작한다면 세션을 **조회**하는 단계부터 실패한 것입니다 — 이 서비스도 어느 세션인지 모르므로, 기록된 IP와 시각으로 관리 → 세션에서 찾아 종료하고 데이터베이스 상태를 먼저 확인하세요. |
...
**잠금 안내를 봤다는 문의가 곧 잠김을 뜻하지는 않습니다.** 로그인 화면은 세 번 거절되면 「반복 실패하면 계정이 일정 시간 잠기며…」를 스스로 띄우는데(계정 존재 여부를 노출하지 않기 위해 브라우저가 세고, 서버는 잠금 여부를 답하지 않습니다), 그 카운터는 **자격증명을 실제로 대조해서 거절한 답(`401`)만** 셉니다. 다음 두 가지는 세지 않으므로, 사용자가 이 안내를 봤다면 잠금 정책이 실제로 걸린 것입니다:

- `403 account_mismatch` — 로그인은 **성공했고** 인가 코드만 발급하지 않은 경우입니다. 서버는 그 직전에 실패 횟수를 초기화했으므로 이 계정은 잠기지 않습니다. 사용자에게는 「연결한 서비스로 돌아가지 마세요. 이 화면에서 요청한 계정으로 다시 로그인하면 그대로 이어집니다.」가 함께 보이며, 그것이 맞는 안내입니다 — 인가 요청은 소진되지 않았습니다. RP가 오래된 `id_token_hint`를 계속 보내는지 확인하세요(위 `result=PARTIAL` 항목).
- `500`·연결 실패 — 이쪽이 시도를 끝내지 못한 것이고 계정에는 아무것도 기록되지 않았습니다. (`500 authorization_code_failed`만은 반대로 로그인이 **끝난** 경우입니다 — 아래 «로그인 화면이 로그인 요청을 읽지 못할 때»의 두 번째 표.)

원래는 셋 다 같이 세어서, **비밀번호가 맞았던 사람에게 비밀번호를 의심하고 잠기기 전에 관리자에게 문의하라고 안내**했습니다. 잠길 일이 없는 계정에 대해서였습니다.

## 로그인 화면이 로그인 요청을 읽지 못할 때

RP에서 넘어온 로그인 화면은 폼을 그리기 전에 `GET /api/v1/auth/challenge/{token}`으로 그 요청을 읽습니다. 이 호출이 실패했을 때 사용자가 보는 안내는 두 가지이고, **문의를 받았을 때 둘을 구분해서 들어야 합니다.**

| 사용자가 본 문구 | 원인 | 사용자가 할 일 |
|---|---|---|
| 「로그인 요청이 만료되었습니다. 연결한 서비스에서 다시 시작하세요.」 | `404` — request token이 소진·만료되었거나 발급된 적이 없습니다 | RP에서 다시 시작 |
| 「로그인 요청을 확인하지 못했습니다. …잠시 후 다시 시도하세요.」 + `다시 시도` 버튼 | 이쪽 문제입니다 — 저장소가 답하지 않아 `500`이거나(서버 로그와 화면의 Trace ID로 찾습니다), 재시작 중이라 연결되지 않았습니다 | 그 자리에서 `다시 시도` |

**두 번째 문구가 보고되면 사용자를 RP로 돌려보내지 마세요.** 원래는 어느 실패든 첫 번째 문구만 보여줬는데, 그것은 설명이 아니라 지시입니다 — 사용자는 RP로 돌아가 새 request token을 받고 같은 장애를 다시 만나며, 정작 들고 있던 요청은 소진되지 않은 채 그대로 남아 있습니다. 장애가 걷힌 뒤 `다시 시도`를 누르면 같은 요청으로 그 화면에서 흐름이 이어집니다.

폼을 제출한 뒤(`POST /api/v1/auth/login`)에도 같은 구분이 있습니다. 이 경로의 `500`은 두 가지이고 **반대의 지시**를 줍니다 — 한때 둘 다 `internal_error`라 화면이 구분할 수 없었습니다:
...
1. 새 RSA-3072 키 생성
2. 새 키를 `ACTIVE`로 전환
3. 기존 키를 `PASSIVE`로 전환하고 **2시간 동안** JWKS에 함께 제공
4. 새 Token은 새 `kid`로 서명
5. 2시간이 지나면 이전 키는 JWKS에서 사라지고 그 키로 서명된 Token은 더 이상 검증되지 않습니다

JWKS 응답은 `Cache-Control: public, max-age=30`으로 제공됩니다. 이 값은 인스턴스가 키 집합을 캐시하는 시간과 같으므로, RP나 중간 Proxy가 새 `kid`가 없는 키 집합을 들고 있을 수 있는 시간은 회전 직후 최대 30초입니다. 그보다 오래 캐시하도록 Reverse Proxy에서 이 헤더를 덮어쓰지 마세요.

서명 키는 자동으로 회전되지 않습니다. 콘솔은 활성 키가 180일을 넘기면 대시보드 준비 상태와 서명 키 화면에서 회전을 권고하며, 이는 실패가 아니라 권고이므로 키는 계속 정상 동작합니다. 회전 후 이전 공개키는 기존 Token 검증을 위해 2시간 동안 JWKS에 유지되므로 서비스 중단 없이 수행할 수 있습니다. 이 시각은 키 레코드의 `retire_at`이며, 지난 뒤에는 보존 정리 실행 여부와 무관하게 즉시 효력을 잃습니다.
