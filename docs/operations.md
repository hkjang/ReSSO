...
- `resso_client_auth_failures_total` 급증 — Client Secret 오설정 또는 대입 시도. 이 계열은 **자격증명을 실제로 대조해서 거절한 것만** 셉니다. Client를 조회하지 못했거나 저장된 Secret을 읽지 못한 경우는 대조 자체가 이뤄지지 않았으므로 여기 잡히지 않고, Token·Introspection·Revocation이 401 `invalid_client` 대신 500 `server_error`로 답합니다(`resso_http_requests_total{status="500"}`과 서버 로그의 `client authentication could not be decided`). 401로 답하면 설정이 멀쩡한 RP가 자기 Secret이 거절당했다고 읽고, 그 실패가 대입 방지 카운터에 쌓여 장애가 끝난 뒤에도 5분간 429가 나갑니다.
