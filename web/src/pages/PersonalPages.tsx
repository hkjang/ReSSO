...
export function PersonalSecurityPage() {
  ...
  const change = useMutation({
    mutationFn: () => api<void>('/api/v1/me/password', { method: 'PUT', ...jsonBody({ current_password: current, new_password: replacement }) }),
    onSuccess: () => { setCurrent(''); setReplacement(''); setConfirm(''); notify('비밀번호를 변경하고 다른 세션을 종료했습니다.') },
  })
...
export function APIKeysPage() {
  ...
  const rotate = useMutation({ mutationFn: (key: APIKey) => api<{ key: APIKey; secret: string }>(`/api/v1/me/api-keys/${key.id}/rotate`, { method: 'POST' }), onSuccess: async (result) => { setSecret(result.secret); await invalidate() } })
