...
export function AuditPage() {
  ...
  return <><PageHeader title="감사 이벤트" description="로그인, Token, 관리자 변경과 키 회전을 보안 감사 관점에서 추적합니다." badge="365일 보존" />
    ...
    <ContentCard noPadding>{query.isLoading ? <PageLoading /> : query.error ? <Box sx={{ p: 2 }}><ErrorAlert error={query.error} /></Box> : !query.data?.items.length ? <EmptyState title={filtered ? '조건에 맞는 감사 이벤트가 없습니다' : '감사 이벤트가 없습니다'} description={filtered ? '필터를 해제하거나 조건을 넓혀보세요.' : undefined} /> : <>
