...
// Matches the console's dashboard advisory. Signing keys never rotate on
// their own, so the page has to say when one has aged.
const ADVISORY_DAYS = 180

function ageInDays(createdAt: string): number {
  return Math.floor((Date.now() - new Date(createdAt).getTime()) / 86_400_000)
}

function KeyAge({ createdAt }: { createdAt: string }) {
  const days = ageInDays(createdAt)
  return <Typography variant="body2" color={days > ADVISORY_DAYS ? 'warning.main' : 'text.primary'}>{days.toLocaleString()}일</Typography>
}
...
export function KeysPage() {
  ...
  const agingKey = activeKey ? ageInDays(activeKey.created_at) > ADVISORY_DAYS : false
