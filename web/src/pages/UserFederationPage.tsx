        ...
        <Stack spacing={3}>
          ...
          {editing && liveEditing && <><Divider /><Section title="연결 검증 및 동기화" description="저장된 설정으로 실제 LDAP 서버에 연결합니다. 인증 테스트 비밀번호는 저장하거나 감사로그에 기록하지 않습니다.">
            <Stack spacing={2}>
              ...
              {(liveEditing.last_sync_unknown_roles?.length ?? 0) > 0 && !syncRunning && <Alert severity="warning" sx={{ mt: 1 }}>Group 매핑이 가리키는 Role이 이 Realm에 없습니다: {liveEditing.last_sync_unknown_roles?.join(', ')}. 해당 그룹의 사용자는 그 Role을 받지 못하며, 동기화는 성공으로 끝나므로 오류로 표시되지 않습니다.</Alert>}
              {Object.keys(liveEditing.group_role_mappings).length > 0 && liveEditing.last_sync_at && liveEditing.last_sync_group_memberships === 0 && !syncRunning && <Alert severity="warning" sx={{ mt: 1 }}>Group 매핑이 설정돼 있지만 이번 동기화에서 그룹 소속을 가진 사용자가 한 명도 없었습니다. `memberOf` 속성 이름이 맞는지, 디렉터리에 그 속성을 만들어 주는 overlay가 켜져 있는지 확인하세요 (OpenLDAP은 기본으로 켜지지 않습니다). 매핑이 옳아도 대조할 것이 없으면 아무 Role도 부여되지 않습니다.</Alert>}
