...
MCP 도구는 기본적으로 읽기 전용이며 Secret이나 Private Key를 반환하지 않습니다. 사용자 검색은 한 번에 20건까지만 돌려주며, 조건에 맞는 전체 수(`matched`)와 잘렸는지(`truncated`)를 함께 알려줍니다 — 잘린 목록을 전체로 읽지 않도록 하기 위한 것입니다.
